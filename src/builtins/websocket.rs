...
fn ws_ctor(ctx: &mut JSContext, args: &[JSValue]) -> JSValue {
    ...
    let url = match Url::parse(&url_str) {
        Ok(u) => u,
        Err(e) => { eprintln!("WebSocket: invalid URL: {e}"); return JSValue::undefined(); }
    };

    let use_tls = url.is_tls();
    let rx = match Connection::connect_async(url.host.clone(), url.port, use_tls) {
        Ok(r) => r,
        Err(e) => { eprintln!("WebSocket: connect failed: {e}"); return JSValue::undefined(); }
    };

    let mut conn = match rx.recv() {
        Ok(Ok(c)) => c,
        _ => { eprintln!("WebSocket: connection failed"); return JSValue::undefined(); }
    };
    ...
    if conn.write_all(req_str.as_bytes()).is_err() {
        eprintln!("WebSocket: handshake write failed"); return JSValue::undefined();
    }
    ...
    loop {
        ...
        match conn.read(&mut tmp) {
            ...
            Err(e) => { eprintln!("WebSocket: handshake read error: {e}"); return JSValue::undefined(); }
        }
    }
    let resp_str = String::from_utf8_lossy(&resp_buf);
    let accept_line = resp_str.lines().find(|l| l.to_lowercase().starts_with("sec-websocket-accept:"));
    let accept = accept_line.and_then(|l| l.splitn(2, ':').nth(1)).map(|s| s.trim()).unwrap_or_default();
    if !WsHandshake::verify_accept(&key, accept) {
        eprintln!("WebSocket: accept mismatch"); return JSValue::undefined();
    }
    if !resp_str.contains("101") {
        eprintln!("WebSocket: expected 101"); return JSValue::undefined();
    }
...
fn ws_send(ctx: &mut JSContext, args: &[JSValue]) -> JSValue {
    ...
    if idx == usize::MAX { return JSValue::undefined(); }
...
fn ws_close(ctx: &mut JSContext, args: &[JSValue]) -> JSValue {
    ...
    let code = if args.len() > 1 && args[1].is_int() { args[1].get_int() as u16 } else { 1000 };
    ...
    if args[0].is_object() {
        args[0].as_object_mut().set(ctx.intern("readyState"), JSValue::new_int(3));
    }
